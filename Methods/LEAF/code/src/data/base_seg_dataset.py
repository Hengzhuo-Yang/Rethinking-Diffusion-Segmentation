...
def threshold_mask_tensor(x: torch.Tensor) -> torch.Tensor:
    return torch.where(x > 128, 255.0, 0.0)
...
class BaseSegDataset(Dataset):
    ...
    def __init__(
        ...
        super().__init__()
        if int(num_seg_classes) != 2:
            raise ValueError(
                "BaseSegDataset is a binary-mask loader: it reads masks as RGB and thresholds them to 0/1. "
                "For multi-class segmentation, implement a dataset loader that preserves class-index labels "
                "or one-hot masks before training."
            )
        ...
        mask_transforms = transforms.Compose(
            [
                ...
                transforms.Lambda(threshold_mask_tensor),
        ...
        # Load base filenames
        base_filenames = sorted(os.listdir(os.path.join(self.dataset_dir, self.rgb_subfolder)))

"""Preprocess shared raw ACDC NIfTI frames into a LEAF-owned PNG cache.
...
DEFAULT_MANIFEST_ROOT = Path(__file__).resolve().parents[1] / "manifests" / "acdc"
...
def load_split(split_root: str | Path) -> dict[str, list[str]]:
    ...
    split = {
        "training": read_patient_list(root / "train.txt"),
        "validation": read_patient_list(root / "val.txt"),
        "testing": read_patient_list(root / "test.txt"),
    }
    all_patients = [patient for patients in split.values() for patient in patients]
    for split_name, patients in split.items():
        if _manifest_fingerprint(patients) != EXPECTED_MANIFEST_FINGERPRINTS[split_name]:
            raise ValueError(
                f"ACDC {split_name} manifest differs from the fixed release manifest: {root}"
            )
...
def foreground_rgb_from_label(label: np.ndarray, num_classes: int) -> np.ndarray:
    if num_classes != 4:
        raise ValueError("ACDC LEAF foreground RGB encoding expects num_classes=4")
...
def preprocess(args: argparse.Namespace) -> None:
    ...
    split = load_split(args.manifest_root)
    ...
    if {key: len(value) for key, value in split.items()} != EXPECTED_PATIENT_COUNTS:
        raise ValueError(
            f"ACDC manifests do not match fixed 70/10/20 split: {patient_counts}"
        )
    ...
    if slice_counts != EXPECTED_SLICE_COUNTS:
        raise RuntimeError(f"ACDC slice counts differ from release contract: {slice_counts}")
    ...
    summary = {
        ...
        "split_note": (
            "Uses the documented fixed 70/10/20 patient split files "
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--raw_root", required=True, help="Raw ACDC database or a parent containing training.")
    parser.add_argument(
        "--manifest_root",
        default=str(DEFAULT_MANIFEST_ROOT),
        help="Directory containing fixed train.txt, val.txt, and test.txt patient manifests.",
    )
    parser.add_argument("--output_root", required=True, help="New LEAF-owned ACDC output directory.")

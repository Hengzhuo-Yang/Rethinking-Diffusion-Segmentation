"""Evaluate a LEAF checkpoint on the LEAF-owned ISIC2018 Task 1 PNG cache."""
...
def dice_iou(pred: torch.Tensor, target: torch.Tensor):
    pred = pred.bool()
    target = target.bool()
...
def save_mask_png(mask: torch.Tensor, path: Path) -> None:
    array = mask.detach().cpu().clone().squeeze().numpy().astype(np.uint8) * 255
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    parser.add_argument(
        "--split",
        default="testing",
        choices=["train", "training", "val", "valid", "validation", "test", "testing"],
    )
    ...
    num_classes = int(cfg.get("num_seg_classes", 2))
    if num_classes != 2:
        raise ValueError(f"ISIC2018 evaluation expects num_seg_classes=2, got {num_classes}")
    ...
    for batch in dataloader:
        ...
        if rgb.device.type != "cuda" or pred.device.type != "cuda":
            raise RuntimeError("ISIC2018 final inference left CUDA; CPU fallback is forbidden.")
        ...
        for batch_idx in range(pred.shape[0]):
            ...
            save_mask_png(pred_i, sample_dir / f"{image_id}_pred.png")
            rows.append(
                {
                    "image_id": image_id,
    ...
    summary = {
        ...
        "inference_time_per_image_s": inference_seconds / max(len(rows), 1),
        ...
        "metric_policy": "foreground Dice/IoU for binary lesion masks; skip empty ground-truth images",
    }

    if not rows:
        raise RuntimeError(f"No rows evaluated for split={args.split}")
    write_csv(args.per_image_csv, rows, list(rows[0].keys()))

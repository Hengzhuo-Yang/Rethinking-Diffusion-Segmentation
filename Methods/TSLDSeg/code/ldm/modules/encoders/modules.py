...
class AEEncoderEmbedder(AbstractEncoder):
    ...
    def __init__(self, **ddconfig):
        ...
        self.oeem = TAM(in_channels=ddconfig['init_ch'] * 4, out_channels=scale_factor * ddconfig["z_channels"])
        self.hsem = HSEM(in_channels=ddconfig['init_ch'] * 4,
                         out_channels=scale_factor * ddconfig["z_channels"], dist_type='cosine')
        self.hff = MCF(ch_1=scale_factor * ddconfig["z_channels"],
                       ch_2=scale_factor * ddconfig["z_channels"],
                       r_2=1,
                       ch_int=scale_factor * ddconfig["z_channels"],
                       ch_out=scale_factor * ddconfig["z_channels"])
        self.out = torch.nn.Conv2d(scale_factor * ddconfig["z_channels"], self.embed_dim, 1)  # default=4*2->4

    def forward(self, x):
        x, mid_f = self.encoder(x)
        oe_x = self.oeem(mid_f)
        hs_x = self.hsem(mid_f)
        x = self.hff(hs_x, oe_x, x)
        x = self.out(x)
        return x
...
if __name__ == "__main__":
    ...
    model = AEEncoderEmbedder(init_ch=128, in_channels=3, out_ch=3, num_res_blocks=2, z_channels=4,
                              embed_dim=4, resolution=256, attn_resolutions=[], double_z=True).cuda()
    z = model(x)
    print(z.shape)

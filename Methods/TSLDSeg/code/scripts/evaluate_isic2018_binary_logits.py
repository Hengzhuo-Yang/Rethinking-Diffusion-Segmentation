...
def main():
    ...
    parser.add_argument("--data_dir", required=True, help="ISIC2018 split directory containing images/ and masks/.")
    ...
    if evaluated == 0:
        raise RuntimeError("No non-empty GT ISIC2018 images were available for Dice/IoU evaluation.")

    summary = {
        "data_dir": str(data_dir),
        "sample_dir": str(sample_dir),
        "threshold": args.threshold,
        "metric_policy": "binary lesion foreground Dice/IoU; skip empty-GT foreground images; exclude background",
        "num_images": len(rows),
        "evaluated_non_empty_gt_images": evaluated,
        "skipped_empty_gt_images": empty_gt,
        "dice_mean": float(np.mean(dice_values)),
        "iou_mean": float(np.mean(iou_values)),
    }

    Path(args.per_image_csv).parent.mkdir(parents=True, exist_ok=True)

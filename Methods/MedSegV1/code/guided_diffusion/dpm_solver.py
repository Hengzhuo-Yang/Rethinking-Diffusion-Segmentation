...
class DPM_Solver:
    ...
    def dynamic_thresholding_fn(self, x0, t):
        ...
        x0 = torch.maximum(torch.minimum(x0, s), -s) / s
        return x0
    ...
    def data_prediction_fn(self, x, t):
        ...
        if noise.shape[1] < x.shape[1]:
            raise ValueError(
                "The diffusion model predicted fewer noise channels "
                f"({noise.shape[1]}) than the sampled mask has ({x.shape[1]})."
            )
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        sample = x
        cal = None
        out = self.model(torch.cat((self.img, sample), dim=1).to(dtype = torch.float), t)
        if isinstance(out, tuple):
            _, cal = out
        if return_intermediate:

...
class UNetModel_v1preview(nn.Module):
    ...
    def __init__(
        ...
        self.segmentation_channels = int(segmentation_channels)
        self.core_output_channels = int(core_output_channels)
        ...
        if high_way:
            ...
            self.hwm = Generic_UNet(
                self.in_channels - self.segmentation_channels,
                features,
                self.core_output_channels,
                5,
                highway=True,
            )
    ...
    def highway_forward(self, x, hs):
        # V1 uses Generic_UNet as a condition encoder only. Stop at its
        # bottleneck instead of executing the merged-code localization head.
        return self.hwm(x, hs, encoder_only=True)

    def core_no_diff_logits(self, image):
        hwm_dtype = next(self.hwm.parameters()).dtype
        _emb, logits = self.hwm(image.to(dtype=hwm_dtype), hs=None, return_logits=True)
        return logits
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = x.type(self.dtype)
        c = h[:, :-self.segmentation_channels, ...]
        hlist= []
        ...
        h = h.type(x.dtype)
        out = self.out(h)
        # Keep the historical two-item API for downstream diffusion helpers,
        # while explicitly reporting that V1 has no calibration-mask output.
        return out, None
...
class UNetModel_newpreview(nn.Module):
    ...
    def core_no_diff_logits(self, image):
        hwm_dtype = next(self.hwm.parameters()).dtype
        _anch, logits = self.hwm(image.to(dtype=hwm_dtype), hs=None, return_logits=True)
        return logits
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, hs=None, return_logits=False, encoder_only=False):
        skips = []
        seg_outputs = []
        anch_outputs = []
        ...
        # Keep localization modules registered so old checkpoints still load
        # strictly, but do not execute them on the paper-aligned V1 path.
        if encoder_only:
            return emb
        ...
        for u in range(len(self.tu)):
            ...
            if self._deep_supervision:
                seg_logits = self.seg_outputs[u](x)
                seg_outputs.append(seg_logits if return_logits else self.final_nonlin(seg_logits))
            if self.anchor_out and (not self._deep_supervision):
                anch_outputs.append(x)
        if not seg_outputs:
            seg_logits = self.seg_outputs[0](x)
            seg_outputs.append(seg_logits if return_logits else self.final_nonlin(seg_logits))

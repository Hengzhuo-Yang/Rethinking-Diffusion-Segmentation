...
def main():
    ...
    if not mask_paths:
        raise RuntimeError(f"No ISIC2018 masks found under {data_dir / 'masks'}")

    rows = []
    pred_threshold = int(round(args.pred_threshold * 255.0))
    for mask_path in mask_paths:
        image_id = mask_path.stem
        pred_path = sample_dir / f"{image_id}_pred.png"
        if not pred_path.exists():
            raise FileNotFoundError(f"Missing prediction for {image_id}: {pred_path}")

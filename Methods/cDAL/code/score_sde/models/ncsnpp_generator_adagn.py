...
class NCSNpp(nn.Module):
    ...
    def _skip_timestep_embedding_modules(self):
        m_idx = 0
        if self.embedding_type == 'fourier':
            m_idx += 1
        elif self.embedding_type == 'positional':
    ...
    def forward_core_no_diff(self, y, z):
        """Diffusion-free cDAL core: condition image plus paper latent z only."""
        zemb = self.z_transform(z)
        modules = self.all_modules
        m_idx = self._skip_timestep_embedding_modules()
        temb = None

        # Condition encoder. This is image-derived and is retained.
        h = y
        for _ in range(self.cond_enc_layer_sizes + 1):
            h = modules[m_idx](h)
            m_idx += 1

        # Skip the noisy-mask input convolution. The U-Net starts from image
        # condition features, preserving latent-z AdaGN but removing Y_t and t.
        hs = [h]
        m_idx += 1

...
def parse_args():
    parser = argparse.ArgumentParser("Evaluate cDAL on binary ISIC2018 Task 1")
    parser.add_argument("--parameters", default="parameters_isic2018.json")
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--checkpoint_metadata", required=True)
    parser.add_argument("--data_dir", default="data_preprocessed/isic2018_task1_cdal_png")
...
def main():
    ...
    if int(args.num_channels) != 1 or int(args.num_channels_disc) != 2:
        raise ValueError("ISIC2018 cDAL evaluation requires num_channels=1 and num_channels_disc=2")
    ...
    if cli.overwrite:
        for target in [Path(cli.summary_csv), Path(cli.summary_json), Path(cli.per_image_csv)]:
            if target.exists():
                target.unlink()
    ...
    summary.update(
        {
            ...
            "per_image_csv": str(cli.per_image_csv),
            "dice": dice,
            "miou": miou,
        }
    )

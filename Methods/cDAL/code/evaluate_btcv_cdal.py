...
def parse_args():
    parser = argparse.ArgumentParser("Evaluate cDAL on binary 2D BTCV")
    parser.add_argument("--parameters", default="parameters_btcv.json")
    parser.add_argument("--model_path", required=True)
    parser.add_argument("--checkpoint_metadata", required=True)
    parser.add_argument("--data_dir", default="data_preprocessed/btcv_synapse_cdal_binary_png")
    parser.add_argument("--manifest", required=True)
    parser.add_argument("--split", choices=["test", "testing"], default="test")
    parser.add_argument("--output_folder", required=True)
    parser.add_argument("--summary_csv", required=True)
    parser.add_argument("--summary_json", required=True)
    parser.add_argument("--per_slice_csv", required=True)
    parser.add_argument("--local_rank", type=int, default=0)
    parser.add_argument("--major_vote_number", type=int, default=30)
